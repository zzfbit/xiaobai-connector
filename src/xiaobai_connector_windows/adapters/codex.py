"""Windows Codex app-server v2 adapter.
...
class CodexAdapter:
    ...
    async def queue_thread_message(self, *, thread_id: str, text: str,
        ...
        """Deliver a phone message into a desktop-owned Windows Codex thread.

        The Windows ChatGPT/Codex desktop app owns the long-lived writer for
        desktop-created threads. Use its app-tools named-pipe bridge first so
        a phone message reaches that writer while the turn is active. The
        durable ``thread/queue/add`` app-server path remains the restart-safe
        fallback when the desktop bridge is unavailable.
        """
        if not _codex_available(self.binary):
