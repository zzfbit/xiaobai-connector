"""Conservative Windows Agent discovery.
...
from .codex_sessions import desktop_codex_binary, resolve_codex_binary
...
def _is_usable_executable(path: Path) -> bool:
    ...
    return path.is_file() and path.suffix.lower() in WINDOWS_EXECUTABLE_SUFFIXES
...
def _codex_binary() -> str | None:
    """Find the Windows Codex executable."""
    return desktop_codex_binary() or _which(("codex",))
...
def scan_agents(configured_agents: list[dict[str, object]] | None = None) -> list[AgentCandidate]:
    ...
    codex = resolve_codex_binary(codex_binary or None)
    codex_candidate = _candidate(
        "codex", "Codex", ("codex",), configured_binary=codex,
        capabilities=("chat", "stream", "cancel", "steer"))
    if codex and (not codex_binary or codex != codex_binary):
