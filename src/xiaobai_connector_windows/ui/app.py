"""Windows-only Tkinter setup wizard.
...
from xiaobai_connector_windows.config import ConnectorConfig, VALID_SANDBOXES
from xiaobai_connector_windows.credentials import CredentialStore, SERVICE
from xiaobai_connector_windows.discovery import AGENT_DISPLAY_NAMES, resolve_executable, scan_agents
from xiaobai_connector_windows.pairing import PairingClient, PairingError, PairingPending, PairingRequest
from xiaobai_connector_windows.paths import ensure_data_dir, log_path
from xiaobai_connector_windows.runtime import ConnectorRuntime
...
class ConnectorApp(tk.Tk):
    def __init__(self):
        ...
        self._polling = False
        self._scan_queue: queue.Queue[tuple[int, str, Any]] = queue.Queue()
        self._scan_generation = 0
        self._scan_poll_id: str | None = None
    ...
    def _invalidate_scan(self) -> None:
        ...
        if self._scan_poll_id is not None:
            try:
                self.after_cancel(self._scan_poll_id)
            except tk.TclError:
                pass
            self._scan_poll_id = None
    ...
    def _scan_async(self) -> None:
        ...
        while True:
            try:
                ...
                break
        threading.Thread(target=self._scan_worker, args=(generation,),
                         name="agent-discovery", daemon=True).start()
        self._scan_poll_id = self.after(
            80, lambda generation=generation: self._poll_scan_result(generation))

    def _scan_worker(self, generation: int) -> None:
        try:
            result = scan_agents(self.config_value.agents)
            self._scan_queue.put((generation, "ok", result))
        except Exception as exc:
            self._scan_queue.put((generation, "error", str(exc)))

    def _poll_scan_result(self, generation: int) -> None:
        if generation != self._scan_generation:
            return
        if (not self.winfo_exists() or not hasattr(self, "agent_frame")
                or not self.agent_frame.winfo_exists()):
            return
        self._scan_poll_id = None
        try:
            result_generation, state, value = self._scan_queue.get_nowait()
        except queue.Empty:
            self._scan_poll_id = self.after(
                80, lambda generation=generation: self._poll_scan_result(generation))
            return
...
def main() -> None:
    if platform.system() != "Windows":
        raise RuntimeError("Windows Connector 只能在 Windows 上运行")
    ...
    if "--message-agent-mcp" in sys.argv:
        from xiaobai_connector_windows.message_agent_mcp import main as mcp_main
