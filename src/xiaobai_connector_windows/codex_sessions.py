"""Read-only bridge to the Windows Codex session catalogue.
...
DESKTOP_CODEX_INSTALL = ("OpenAI", "Codex", "bin")
CODEX_COMMAND_SHIMS = frozenset({"codex", "codex.cmd", "codex.bat", "codex.ps1"})
...
def desktop_codex_binary() -> str | None:
    """Return the newest Codex executable bundled with the Windows app.

    The desktop app keeps its active CLI under a content-addressed child
    directory.  The npm ``codex.cmd`` shim may point at an older CLI release,
    and it is not the binary the desktop app uses for its local session store.
    """
    local_app_data = os.environ.get("LOCALAPPDATA")
    root = (Path(local_app_data) if local_app_data
            else Path.home() / "AppData" / "Local")
    root = root.joinpath(*DESKTOP_CODEX_INSTALL)
    candidates: list[Path] = []
    try:
        versioned = [
            item / "codex.exe" for item in root.iterdir()
            if item.is_dir() and (item / "codex.exe").is_file()
        ]
        versioned.sort(key=lambda item: item.stat().st_mtime, reverse=True)
...
def _is_command_shim(value: str) -> bool:
    raw = str(value or "").strip().strip('"')
    if not raw:
        return False
    path = Path(raw).expanduser()
    name = path.name.lower()
    if name in CODEX_COMMAND_SHIMS:
        return True
    # A bare command can resolve to codex.cmd/codex.ps1 depending on whether
    # the caller is Explorer, cmd.exe, or PowerShell.
    return not path.is_absolute() and name == "codex"
...
def resolve_codex_binary(binary: str | None = None) -> str:
    ...
    desktop = desktop_codex_binary()
    if desktop and (not configured or _is_command_shim(configured)):
        return desktop
    if configured:
        configured_path = Path(configured).expanduser()
        if configured_path.is_file() or shutil.which(configured):
            return configured
    if desktop:
        return desktop
    ...
    def _request(self, method: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            proc = subprocess.Popen(
                ...
                text=True,
                # Codex app-server speaks UTF-8 JSON-RPC.  Do not let the
                # machine's active ANSI code page (GBK on many Windows
                # installs) decode stdout before the JSON parser sees it.
                encoding="utf-8",
                **subprocess_options(),
            )
...
__all__ = [
    "CodexSessionClient", "CodexSessionError", "desktop_codex_binary",
